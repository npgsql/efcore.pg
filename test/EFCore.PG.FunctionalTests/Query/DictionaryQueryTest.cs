...
SELECT s."Id", s."Dictionary", s."ImmutableDictionary", s."JsonDictionary", s."JsonbDictionary"
...
""");
    ...
    public async Task JsonDictionary_Concat_JsonDictionary(bool async)
    {
        ...
        AssertSql(
            """
SELECT s."JsonDictionary"::jsonb || s."JsonbDictionary"::jsonb
FROM "SomeEntities" AS s
""");
    }
...
""");
    ...
    public async Task Extensions_ValuesForKeys(bool async)
    {
        string[] keys = ["key"];
        await AssertQuery(
            async, ss => ss.Set<DictionaryEntity>().Select(s => EF.Functions.ValuesForKeys(s.Dictionary, keys)),
...
""");
    ...
    public async Task Extensions_ValuesForKeys_JsonDictionary(bool async)
    {
        string[] keys = ["key"];
        await AssertQuery(
            async, ss => ss.Set<DictionaryEntity>().Select(s => EF.Functions.ValuesForKeys(s.JsonDictionary, keys)),
...
""");
    ...
    public async Task Extensions_ValuesForKeys_JsonbDictionary(bool async)
    {
        string[] keys = ["key"];
        await AssertQuery(
            async, ss => ss.Set<DictionaryEntity>().Select(s => EF.Functions.ValuesForKeys(s.JsonbDictionary, keys)),
            ss => ss.Set<DictionaryEntity>().Select(
                s
                    // ReSharper disable once CanSimplifyDictionaryLookupWithTryGetValue
#pragma warning disable CA1854
